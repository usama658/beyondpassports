{{-- Services hero (shared). Prose (eyebrow/title/lede) is CMS-editable; the "Start here" card
     and steps are structural. Used by the coded services page AND the CMS hero block, so both
     render identically. --}}
@php
  $waNumber = config('ukv.whatsapp') ?: '447462230788';
  $waFor = fn (string $t): string => 'https://wa.me/'.$waNumber.'?text='.rawurlencode("Hi Beyond Passports, I'd like help with: ".$t);
  ...
  $title = $title ?? 'Visa and travel services, all in one place';
  $lede = $lede ?? 'Tell us where you are going. We sort the visa, the documents and the appointment so your trip goes ahead.';
@endphp
