...
@php $bpcWa = config('ukv.whatsapp') ?: '4915213103462'; @endphp
<form class="bpc-af" onsubmit="return bpcAppt(this)" data-capture="{{ route('appointment.enquiry') }}" novalidate>
  ...
  <p class="bpc-af-sub">Just your name and number. A <x-reg-verify>England and Wales registered</x-reg-verify> service spots what could get you refused, then holds the soonest slot before it goes.</p>
  ...
  <a class="bpc-af-go" href="https://wa.me/{{ $bpcWa }}?text={{ rawurlencode('Hi Beyond Passports, I would like to check my Schengen visa eligibility.') }}" target="_blank" rel="noopener" data-wa="{{ $bpcWa }}" onclick="return bpcAppt(this.closest('form'))">
  ...
  <p class="bpc-af-trust"><span class="bpc-af-dot"></span>@include('partials.uk-eu-flags',['size'=>13]) Reply in minutes · No payment now · <x-reg-verify style="text-decoration:none">Registered in <b>England and Wales</b></x-reg-verify></p>
</form>
<script>
function bpcAppt(f){
  ...
  try{ if(window.bpLead) window.bpLead({name:n,phone:p,email:e,formName:'lp-appt'}); }catch(_){}
