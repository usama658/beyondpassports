...
@section('title', 'Application received | Beyond Passports')
@section('description', 'Your application is in. A UK-based specialist will review your case and send a personalised quote. We are opening WhatsApp so your details reach our team there too.')
...
<style>
  ...
  /* reference bar */
  .tk-ref { display: flex; justify-content: space-between; align-items: center; gap: 12px; background: var(--navy); color: #fff; border-radius: 12px; padding: 13px 16px; margin: 0 0 16px; }
  .tk-ref .l { font: 700 10px var(--body); letter-spacing: .1em; text-transform: uppercase; color: var(--soft); margin: 0 0 3px; }
  .tk-ref .v { font: 800 15px var(--mono, monospace); letter-spacing: .04em; word-break: break-all; color: #fff; }
...
<section class="ct-thanks"><div class="wrap">
  <div class="tk-grid">
    <div class="tk-left">
      ...
      <h1>Thanks{{ $leadName ? ', '.$leadName : '' }}. Your application is in.</h1>
      <p>A UK-based visa specialist will review your case{{ $destination ? ' for '.$destination : '' }} and send a personalised quote, usually within one business day. No payment is taken until you've approved it.</p>
      @if ($orderRef)
      <div class="tk-ref">
        <div><div class="l">Your reference</div><div class="v">{{ $orderRef }}</div></div>
      </div>
      @endif
      <div class="tk-estep">
    ...
    <div class="tk-panel">
      ...
      <p class="sub">The chat opens in <b id="tk-count">3</b> seconds, prefilled with your details.</p>
