...
.vfr .cdir .cdaz{display:flex;flex-wrap:wrap;gap:4px}
.vfr .cdir .cdaz button{border:1px solid var(--edge);background:#fff;color:var(--muted);width:28px;height:28px;border-radius:8px;font:800 11px 'Outfit',sans-serif;cursor:pointer;transition:all .1s}
.vfr .cdir .cdaz button:hover:not(:disabled){border-color:var(--stamp);color:var(--stampt)}
.vfr .cdir .cdaz button.on{background:var(--stampt);border-color:var(--stampt);color:#fff}
.vfr .cdir .cdaz button:disabled{opacity:.35;cursor:default}
.vfr .cdir .cdgrid{display:grid;grid-template-columns:repeat(5,1fr);gap:10px}
.vfr .cdir .cdcard{display:flex;align-items:center;gap:10px;background:#fff;border:1px solid var(--edge);border-radius:12px;padding:11px 13px;text-decoration:none;color:inherit;transition:transform .1s,border-color .1s;box-shadow:0 14px 32px -30px rgba(20,34,46,.5)}
.vfr .cdir .cdcard:hover{transform:translateY(-2px);border-color:var(--stamp)}
.vfr .cdir .cdcard img{width:24px;height:16px;border-radius:3px;object-fit:cover;box-shadow:0 1px 3px rgba(0,0,0,.18);flex:none}
.vfr .cdir .cdcard b{font:800 13px 'Outfit',sans-serif}
.vfr .cdir .cdcard .cdgo{margin-left:auto;color:var(--stampt);opacity:.5;display:inline-flex}
.vfr .cdir .cdcard:hover .cdgo{opacity:1}
.vfr .cdir .cdcard .cdgo svg{width:13px;height:13px;fill:none;stroke:currentColor;stroke-width:2.2;stroke-linecap:round;stroke-linejoin:round}
.vfr .cdir .cdnone{grid-column:1/-1;align-items:center;gap:14px;flex-wrap:wrap;background:var(--paper);border:1px dashed var(--edge);border-radius:14px;padding:16px 18px;display:none}
...
@media(max-width:820px){.vfr .cdir .cdgrid{grid-template-columns:1fr 1fr 1fr}}
@media(max-width:520px){.vfr .cdir .cdgrid{grid-template-columns:1fr 1fr}}
</style>
@endpush
...
<div class="vfr">
    ...
    <div>
      ...
      <div class="pbadges">
        <div class="pbadge">
          ...
          <div class="tx"><b>Start for just &pound;40</b><span>&pound;90 on the full case, after booking</span></div>
        </div>
        <a class="pbadge" href="https://find-and-update.company-information.service.gov.uk/company/{{ config('ukv.company_no') ?: '17331903' }}" target="_blank" rel="noopener" title="Verify our UK registration on Companies House" style="text-decoration:none;color:inherit">
          <span class="sl glow"><span class="wave" aria-hidden="true"></span><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 1 3 5v6c0 5.5 3.8 10.7 9 12 5.2-1.3 9-6.5 9-12V5l-9-4zm-1 15-4-4 1.4-1.4L11 13.2l4.6-4.6L17 10z"/></svg></span>
  ...
  <section class="cdir"><div class="wrap">
    ...
    <div class="cdgrid" id="cd-grid">
      @foreach($cdCountries as $code => $name)
        <a class="cdcard" data-name="{{ strtolower($name) }}" data-l="{{ strtoupper($name[0]) }}" href="{{ $wa }}?text={{ rawurlencode('Hi Beyond Passports, I need visa help and appointments for '.$name.' from the UK.') }}">
          <img src="https://flagcdn.com/{{ $code }}.svg" alt="{{ $name }} flag" width="24" height="16" loading="lazy">
          <b>{{ $name }}</b>
          <span class="cdgo"><svg viewBox="0 0 24 24"><path d="M5 12h14M13 6l6 6-6 6"/></svg></span>
        </a>
      @endforeach
  ...
  <section class="fp"><div class="wrap"><div class="fp-two">
    <div class="dprice">
      ...
      <div class="hero-n"><span class="big">&pound;40</span><span class="cap">to start &middot; &pound;130 total, &pound;90 after booking</span></div>
      <div class="fee">
...
(function(){
  var grid = document.getElementById('cd-grid'); if (!grid) return;
  var inp = document.getElementById('cd-search'), clr = document.getElementById('cd-clear'), az = document.getElementById('cd-az');
  var none = document.getElementById('cd-none'), cards = grid.querySelectorAll('.cdcard'), letter = '';
  function apply(){
    var q = (inp.value || '').trim().toLowerCase(); clr.style.display = q ? 'block' : 'none';
    var total = 0;
    cards.forEach(function(c){
      var hit = (!q || c.getAttribute('data-name').indexOf(q) >= 0) && (!letter || c.getAttribute('data-l') === letter);
      c.style.display = hit ? '' : 'none'; if (hit) total++;
    });
  ...
  az.addEventListener('click', function(e){
    ...
    letter = (letter === L) ? '' : L;
    az.querySelectorAll('button').forEach(function(x){ x.classList.remove('on'); });
    if (letter) { b.classList.add('on'); inp.value = ''; clr.style.display = 'none'; }
    apply();
  });
  inp.addEventListener('input', function(){ letter = ''; az.querySelectorAll('button').forEach(function(x){ x.classList.remove('on'); }); apply(); });
