@php
    ...
    $waNum        = config('ukv.whatsapp') ?: '4915213103462';
...
<main id="main">
    ...
    <div class="wrap">
      <div class="crp-pass reveal">
        <div class="crp-main">
          ...
          <p class="lede">Here's your tailored list, yours to keep. Bookmark this page or send it to yourself below. When you're ready, we'll confirm your exact requirements before you apply.</p>
        </div>
        <div class="crp-stub">
    ...
    @else
      ...
      @if (session('pay_unavailable'))
        <div role="status" style="max-width:760px;margin:0 auto 14px;background:#fff7e6;border:1px solid #f0d9a8;border-radius:12px;padding:14px 18px;font-size:14px;color:#6b4e10">
          <strong>Card payment is being switched on.</strong> Meanwhile, message our UK team on WhatsApp above and we'll sort your checklist right away.
        </div>
      @endif
    ...
    <div class="deliver reveal" id="send">
      ...
      <form method="POST" action="{{ $sendAction }}" novalidate>
        ...
        <div class="channels">
          ...
          <label class="chan">
            <span class="t"><svg viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163a11.867 11.867 0 0 1-1.587-5.946C.16 5.335 5.495 0 12.05 0a11.817 11.817 0 0 1 8.413 3.488 11.824 11.824 0 0 1 3.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 0 1-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 0 0 1.51 5.26l-.999 3.648 3.978-1.607zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg></span>
  ...
  <section class="cr-section"><div class="wrap">
    <div class="share reveal" id="share">
      ...
      <div class="action-row">
        <a href="https://wa.me/?text={{ urlencode('My document checklist for '.$destName.': '.$shareUrl) }}" class="btn--wa"><svg viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163a11.867 11.867 0 0 1-1.587-5.946C.16 5.335 5.495 0 12.05 0a11.817 11.817 0 0 1 8.413 3.488 11.824 11.824 0 0 1 3.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 0 1-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 0 0 1.51 5.26l-.999 3.648 3.978-1.607zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>Share on WhatsApp</a>
  ...
  <section class="cta-band"><div class="wrap reveal">
    ...
    <p style="max-width:52ch;color:#cdd9e1">Start your application and our <x-reg-verify>England and Wales registered</x-reg-verify> service will confirm your exact requirements and check every document before anything is submitted.</p>
    <div class="row">
      ...
      <a href="https://wa.me/{{ config('ukv.whatsapp') ?: '4915213103462' }}?text={{ rawurlencode('Hi Beyond Passports, here is my document checklist, can you check it?') }}" class="btn btn--glass">@include('partials.wa-glyph')Chat on WhatsApp</a>
