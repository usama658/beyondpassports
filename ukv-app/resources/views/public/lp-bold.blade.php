...
.lpb .open{--c:#1F6E63;--cd:#155248;--cbg:#e7f4ef;--cbg2:#f4fbf8;--cbd:#bfe3d8;--soft:#f5faf8}
.lpb .tight{--c:#b5791f;--cd:#9a6413;--cbg:#faeed6;--cbg2:#fffaf0;--cbd:#ecce9a;--soft:#fdf9ef}
.lpb .none{--c:#c0392b;--cd:#992a1f;--cbg:#fbe4e0;--cbg2:#fff3f0;--cbd:#eeb4a8;--soft:#fef4f2}
...
.lpb .ngband{position:relative;overflow:hidden;background:linear-gradient(160deg,#132c34,#1F6E63);border-radius:16px;padding:15px 16px;margin:0 0 14px}
.lpb .ngband::after{content:'';position:absolute;right:-60px;top:-80px;width:220px;height:220px;border-radius:50%;background:radial-gradient(circle,rgba(143,227,201,.18),transparent 65%);pointer-events:none}
.lpb .nghint{position:relative;z-index:1;font-size:14px;font-weight:800;color:#fff;margin:0 0 9px}
.lpb .nghint b{color:#8fe3c9;font-weight:800}
.lpb .nghint span{font-weight:600;color:rgba(255,255,255,.65);font-size:12.5px}
.lpb .ngsearch{position:relative;z-index:2;margin:0}
.lpb .ngsearch>svg{position:absolute;left:14px;top:50%;transform:translateY(-50%);width:16px;height:16px;fill:#1F6E63;pointer-events:none}
.lpb .ngsearch input{width:100%;font:600 14.5px "Outfit",system-ui,sans-serif;color:#16222E;padding:13px 40px;border:0;border-radius:12px;background:#fff;outline:none;transition:box-shadow .15s}
.lpb .ngsearch input:focus{box-shadow:0 0 0 3px rgba(143,227,201,.45)}
.lpb .ngqc{position:relative;z-index:1;display:flex;gap:7px;margin-top:10px;flex-wrap:wrap}
.lpb .ngqc button{font:700 12px "Outfit",system-ui,sans-serif;color:#fff;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.25);padding:6px 12px;border-radius:999px;cursor:pointer;transition:background .15s}
.lpb .ngqc button:hover{background:rgba(255,255,255,.22)}
.lpb .ngqc button.more{color:#8fe3c9;border-color:rgba(143,227,201,.4)}
.lpb .ngmore{position:relative;overflow:hidden;display:flex;align-items:center;gap:14px;flex-wrap:wrap;margin-top:12px;background:linear-gradient(160deg,#132c34,#1F6E63);border-radius:14px;padding:14px 16px;color:#fff}
.lpb .ngmore::after{content:'';position:absolute;left:-60px;bottom:-80px;width:200px;height:200px;border-radius:50%;background:radial-gradient(circle,rgba(143,227,201,.16),transparent 65%);pointer-events:none}
.lpb .ngmore .tx{position:relative;z-index:1;font:800 14px "Outfit",system-ui,sans-serif}
.lpb .ngmore .tx b{color:#8fe3c9}
.lpb .ngmore .tx span{display:block;font:600 12px "Outfit",system-ui,sans-serif;color:rgba(255,255,255,.65);margin-top:2px}
.lpb .ngmore .btns{position:relative;z-index:1;display:flex;gap:9px;margin-left:auto;flex-wrap:wrap}
.lpb .ngmore .btns a,.lpb .ngmore .btns button{display:inline-flex;align-items:center;gap:7px;font:800 12.5px "Outfit",system-ui,sans-serif;padding:10px 15px;border-radius:999px;text-decoration:none;cursor:pointer;white-space:nowrap;transition:transform .12s,filter .15s}
.lpb .ngmore .btns a:hover,.lpb .ngmore .btns button:hover{transform:translateY(-1px);filter:brightness(.97)}
.lpb .ngmore .srch{background:rgba(255,255,255,.12);color:#fff;border:1.5px solid rgba(255,255,255,.3)}
.lpb .ngmore .wam{background:#25D366;color:#fff;border:0}
.lpb .ngmore .wam svg{width:14px;height:14px;fill:#fff;flex:none}
@media(max-width:560px){.lpb .ngmore .btns{margin-left:0;width:100%;flex-direction:column}.lpb .ngmore .btns a,.lpb .ngmore .btns button{width:100%;justify-content:center}}
...
.lpb .ngsug{position:absolute;z-index:30;left:0;right:0;top:calc(100% + 6px);margin:0;padding:6px;list-style:none;background:#fff;border:1px solid var(--edge);border-radius:12px;box-shadow:0 24px 50px -24px rgba(20,34,46,.5);max-height:264px;overflow:auto}
.lpb .ngsug[hidden]{display:none}
.lpb .ngsug li{display:flex;align-items:center;gap:10px;padding:9px 11px;border-radius:8px;font-size:14px;font-weight:600;color:#16222E;cursor:pointer}
.lpb .ngsug li .fl{width:24px;flex:none;display:flex;align-items:center}
.lpb .ngsug .flimg{width:24px;height:16px;object-fit:cover;border-radius:2px;box-shadow:0 0 0 1px rgba(0,0,0,.1);display:block}
.lpb .ngsug li .zt{margin-left:auto;font-size:10px;font-weight:800;letter-spacing:.04em;text-transform:uppercase;color:#8a99a2}
.lpb .ngsug li:hover,.lpb .ngsug li.on{background:#eef4f6;color:#155E7A}
.lpb .ngsug .none{color:#8a99a2;font-weight:500;cursor:default}.lpb .ngsug .none:hover{background:transparent;color:#8a99a2}
...
<section class="hero" id="top"><div class="wrap"><div class="hgrid">
  <div class="hleft">
    ...
    <div class="hcta">
      <a class="hcta-wa" href="{{ $wa }}?text={{ rawurlencode('Hi Beyond Passports, please take my Schengen visa application from here. Where I am going: ') }}">@include('partials.wa-glyph')Take it from here</a>
  ...
  <form class="formcard form" id="lpbCaseForm" autocomplete="off">
    ...
    <div class="combo" id="lpbDest">
      <label for="lpb-dest">Where are you travelling?</label>
...
<section class="sec alt bd{{ config('ukv.slots.week_labels') ? ' wk' : '' }}{{ config('ukv.slots.count_focus') ? ' cf' : '' }}{{ config('ukv.slots.rows') ? ' rows' : '' }}" id="appointments"><div class="wrap">
  <div class="btop"><div><p class="eyebrow">Appointment board</p><h2 class="h2">Schengen Visa Appointment Help - From &pound;40</h2></div><span class="live"><span class="dot"></span><span class="lv-txt">Summer Peak · <span class="lv-mo">Jul–Aug</span></span></span></div>
...
@if(config('ukv.slots.rows'))
  ...
  <div class="ngstage" data-slotboard>
    ...
    <div class="ngband">
      <p class="nghint">Find <b>your country</b> <span>· all 29 Schengen countries covered</span></p>
      <div class="ngsearch">
      ...
      <div class="ngqc" data-slotchips>
        <button type="button">Spain</button><button type="button">France</button><button type="button">Italy</button><button type="button">Germany</button><button type="button">Netherlands</button><button type="button" class="more" data-slotmore>+ 24 more</button>
      </div>
    </div>
    @endif
    <div class="nggrid">
    ...
    <div class="ngmore">
      <div class="tx">Don't see <b>your country</b>?<span>All 29 Schengen countries covered.</span></div>
      <div class="btns">
        <button type="button" class="srch" data-slotfocus>Search your country</button>
        <a class="wam" href="{{ $wa }}?text={{ rawurlencode('Hi Beyond Passports, my Schengen country is not showing on the board. Can you check appointment availability for: ') }}">@include('partials.wa-glyph')Ask us to check yours</a>
      </div>
    </div>
...
@else
  ...
  <div class="bgrid">
    ...
    <a class="hc {{ $c['cls'] }}" href="{{ $wa }}?text={{ rawurlencode($apptMsg) }}" data-slotcountry="{{ $c['name'] }}" data-slotband="{{ ['open' => 'ok', 'tight' => 'lim', 'none' => 'low'][$c['cls']] ?? 'ok' }}" aria-label="Pick a {{ $c['name'] }} appointment slot"><div class="hd"><span class="cty">{{ $c['name'] }}</span><span class="pill">{{ $c['label'] }}</span></div><div class="bd2">@if(config('ukv.slots.count_focus'))<div class="lab">Slots open</div><div class="count">{{ $c['slots'] ?? 0 }}</div><div class="win"><span class="dot"></span><small>In 30 days</small></div>@else<div class="lab">Next available</div>@if(!empty($c['date_rel']))<div class="wk"><span class="rel">{{ $c['date_rel'] }}</span><span class="my">{{ $c['date_my'] }}</span></div>@else<div class="date">{{ $c['date'] }}</div>@endif@if(($c['slots'] ?? 0) > 0)<div class="slots"><span class="n">{{ $c['slots'] }}</span><small>slots in next 30 days</small></div>@endif@endif</div></a>
    @empty
...
@include('partials.board-desktop-form')
...
<section class="pathband" id="paths"><div class="wrap">
  ...
  <div class="sitrow">
    ...
    <a class="sit" data-thx="refusal" href="{{ $wa }}?text={{ rawurlencode('Hi Beyond Passports, I was refused a Schengen visa before. Please review my refusal letter and rebuild my case.') }}"><span class="tx"><span class="st">Previously refused?</span><span class="q">"I was refused before"</span><p class="d">A refusal stays in the shared EU database for 5 years. We read your refusal letter, find the reason the consulate gave, and help to rebuild the case from the ground up.</p></span><span class="scta">Fix my refusal ›</span></a>
  </div>
</div></section>

{{-- FEAR — VIS severity meter (exact refusal-risk design) --}}
<section class="sec fear" id="refusal"><div class="wrap">
  <div class="fsh"><div class="flabel">Why a Schengen visa refusal sticks</div><h2>The Schengen Visa Information System <span class="hl">remembers everything.</span></h2></div>
  <div class="fmeter">
    <div class="row"><div class="num">1</div><div><h3>You get refused</h3><p>You apply for a Schengen visa from the UK. You get refused. That refusal gets logged in a shared EU database called VIS.</p></div><div class="bar"><span></span></div></div>
    <div class="row"><div class="num">2</div><div><h3>29 Schengen countries can see it</h3><p>You get one submission. The moment it's in, it's judged, and if it's wrong, that refusal is on your file across all 29 countries.</p></div><div class="bar"><span></span></div></div>
    <div class="row"><div class="num">3</div><div><h3>It stays for 5 years</h3><p>Not 1. Not 2. Five years on a shared record that follows every future Schengen visa application.</p></div><div class="bar"><span></span></div></div>
...
.lpb .baeintro{color:#a9c2bd;font-size:15.5px;line-height:1.6;margin:0 0 20px}
.lpb .bael .btn{width:auto;align-self:flex-start;padding:14px 22px}
.lpb .baesub{color:#7f9b96;font-size:12px;line-height:1.5;margin:11px 0 0}
...
.lpb .baealso{display:flex;gap:11px;align-items:flex-start;margin:16px 0 0;background:radial-gradient(400px 160px at 12% 0,rgba(21,94,122,.5),transparent),var(--ink2);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:13px 15px;transition:transform .15s ease,box-shadow .15s ease,border-color .15s ease}
@media(hover:hover){.lpb .baealso:hover{transform:translateY(-2px);box-shadow:0 18px 40px -28px rgba(20,34,46,.6);border-color:rgba(127,215,201,.4)}}
.lpb .baealso .ic{flex:none;width:26px;height:26px;border-radius:7px;background:rgba(127,215,201,.16);color:#8ff0c4;display:grid;place-items:center;font-weight:900;font-size:16px;line-height:1}
.lpb .baealso p{margin:0;font-size:13px;line-height:1.55;color:#a9c2bd}.lpb .baealso b{color:#fff}
...
<section class="sec tr" id="trust"><div class="wrap">
  <div class="thead">
    ...
    <h2 class="h2" style="margin:0 auto;max-width:30ch">How Our Schengen Visa Consultancy Works? Four Steps, No Surprises</h2>
  </div>
  <div class="grid">
    <div class="dark">
