...
Route::get('/schengen-visa-agent', fn (\App\Http\Controllers\CmsController $c) => $c->pageOrCoded('schengen-visa-agent', 'public.lp-speed'))->name('lp.speed'); // Speed LP on /schengen-visa-agent; key aligned to URL
...
// Country money pages (premium france-gold template, identity-swapped per country). Served like the
// lp-v2 variants: static public file (public/lp-<country>.html) + injected analytics/consent head.
// Indexable. Whitelisted so it never shadows other routes. Copy refined per country post-launch.
Route::get('/schengen-visa/{country}', function (string $country) {
