...
function ukv_dest_value( $dest, $field ) {
	$allowed = [ 'required_for_uk', 'visa_type', 'max_stay_days', 'govt_fee_gbp',
		...
		'passport_validity_months', 'required_docs' ];
...
// Floating CALL button (main lead channel) — sits left of the WhatsApp button
add_action( 'wp_footer', function () {
	$tel = preg_replace( '/[^0-9+]/', '', (string) get_option( 'ukv_phone_number', '' ) );
	if ( ! $tel ) { return; }
	echo '<a href="tel:' . esc_attr( $tel ) . '" aria-label="Call us" '
		. 'style="position:fixed;bottom:22px;right:90px;z-index:99999;background:#1456B8;width:58px;height:58px;border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 14px rgba(0,0,0,.28)">'
		. '<svg viewBox="0 0 24 24" width="28" height="28" fill="#fff"><path d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.3.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1C10.6 21 3 13.4 3 4c0-.6.4-1 1-1h3.5c.6 0 1 .4 1 1 0 1.2.2 2.4.6 3.6.1.4 0 .8-.2 1l-2.3 2.2z"/></svg></a>';
}, 99 );
